        ...
        **kwargs,
        ...
        for mask_cls_result, mask_pred_result, meta in zip(
            ...
            mask_pred_result = mask_pred_result[
                :, :ori_scaled_height, :ori_scaled_width
            ]
...
class PseudoFeatureAggregator(BaseModule):
    ...
    def forward(self, inputs):
        ...
        assert len(inputs) == 1
        x = inputs[0]
        x = self.channel_fusion(x)  # x: torch.Size([2, 256, 32, 32])

        # # RYH MODI 简化PseudoFeatureAggregator的输出
        # x = inputs[0]
        return x
...
class RSFeatureAggregator(BaseModule):
    ...
    def __init__(
        ...
        model_arch = (
            "base"
            if "base" in in_channels
            else "large"
            if "large" in in_channels
            else "huge"
        )
...
class SAMDet(BaseDetector):
    def __init__(
        self, detector, segmentor, data_preprocessor=None, test_cfg=None, init_cfg=None
    ):
    ...
    def oracle_predict(
        ...
        batch_data_samples = self.detector.predict(
            batch_inputs, batch_data_samples, rescale=rescale
        )
        batch_img_metas = [data_sample.metainfo for data_sample in batch_data_samples]
        for input_img, data_sample, meta in zip(
            batch_inputs, batch_data_samples, batch_img_metas
        ):
            ...
            pred_instance_data.scores = torch.ones_like(
                data_sample.gt_instances.labels,
                dtype=torch.float32,
...
class RSPrompterAnchorMaskHead(FCNMaskHead, BaseModule):
    ...
    def _predict_by_feat_single(
        ...
        im_mask = F.interpolate(
            im_mask.unsqueeze(1), size=(h, w), mode="bilinear", align_corners=False
        ).squeeze(1)
